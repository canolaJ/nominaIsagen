...
const NominaPdfCopy = ({ payRoll,  user }) => (
  <Document>
        ...
        <View style={styles.section}>
          <View style={styles.container}>
            <View style={styles.sectionEmpleadoText}>
                <View>
                  <Text style={[styles.textEmpleado,styles.h1]}>Copia de Nómina</Text>
                  <Text style={styles.textEmpleado}>Fecha : { fecha }</Text>
                  <Text style={styles.textEmpleado}>Código Empleado : { user.id }</Text>
                  <Text style={styles.textEmpleado}>Nombre : { user.nombres }</Text>
                  <Text style={styles.textEmpleado}>Apellidos : { user.apellidos }</Text>
                  <Text style={styles.textEmpleado}>Número de documento : { user.cc }</Text>
                  <Text style={styles.textEmpleado}>Cargo : { user.post }</Text>
                  <Text style={styles.textEmpleado}>Salario Base : { user.salary }</Text>
                </View>
            </View>
          ...
          <View style={styles.sectionEmpleado}>
            ...
            <View style={[ styles.col10]}>
              ...
              <Text style={[styles.col2]}>{ payRoll['holidaysPaid'] ? payRoll['holidaysPaid'] : "0" }</Text>
            </View>
            <View style={[ styles.col10]}>
            ...
            <View style={[ styles.col10]}>
              ...
              <Text style={[styles.col3]}>{ "$" + payRoll['permissionNotPaid'] }</Text>
              <Text style={[styles.col2]}>{ "$ -" + payRoll['permissionNotPaid'] }</Text>
            </View>
            <View style={[ styles.col10]}>
              <Text style={[styles.col8]}>Total Pagado (Total-Deducción)</Text>
              <Text style={[styles.col2]}>{ "$" + payRoll['totalPayRoll'] }</Text>
            </View>
          </View>
        </View>
    </Page>
